...
从保护方案上来看，主流的、受到关注的白盒保护方案共分为三种：基于编码（Encode）的白盒保护方案 [1, 2]、基于高次非线性掩码（Masking）的白盒保护方案 [3, 7] 以及基于虚拟洗牌的白盒保护方案 [4]。
...
| :----------: | :------------------------------: | :------------------------: |
| 运行虚拟内存峰值    |   3.40 MB   |  3.41 MB   |
| 运行物理内存峰值 | 0.64 MB | 0.64MB |
...
| :----------: | :------------------------------: | :------------------------: |:------------------------: |
| 运行虚拟内存峰值    |   2.50 MB   |  2.64 MB   | 2.82 MB |
| 运行物理内存峰值 | 0.52 MB | 0.64 MB | 0.64 MB |
...
slot为1时，并行加密 64 组明文的比特切片方案与 Tongsuo 标准 SM4 的运行内存大小基本相同。
但由于T表实现的缓存机制，多次运行时，布尔加密速度会低于标准实现。
