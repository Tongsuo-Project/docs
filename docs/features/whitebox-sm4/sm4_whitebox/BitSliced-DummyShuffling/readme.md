...
分别测试从并行加密 64 组明文的比特切片实现方式，与Tongsuo SM4 标准实现的速度比较。(默认掩码结构 $x=a \oplus b$)
...
| :----------: | :------------------------------: | :------------------------: |:------------------------: |
| 布尔操作数目 |  -   | 71394  | 154721  |
| 运行内存大小 | 0.11 MB | 0.84 MB | 1.8 MB |
| 加密速度（均值） | 43514.71 KB/s |        1520.89 KB/s         | 696.26 KB/s |
...
slot为1时，并行加密 64 组明文的比特切片方案运行内存大小高于 Tongsuo 标准 SM4 。
由于T表实现的缓存机制，且比特切片方案使用了掩码进一步增强安全性，多次运行时，布尔加密速度会显著低于标准实现。
